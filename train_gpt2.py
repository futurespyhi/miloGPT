...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # attention (materializes the large (T,T) matrix for all the queries and keys) use flash-attention(kernel fusion operation) June 23 2022 written by stanford
...
class GPT(nn.Module):
    ...
    def from_pretrained(cls, model_type):
        ...
        # Always use standard GPT model settings
        ...
        sd = model.state_dict()
        sd_keys = sd.keys()
        # Discard attention bias mask/buffer, not a parameter
        ...
        # Ignore masked_bias, just a buffer
        sd_keys_hf = [k for k in sd_keys_hf if not k.endswith('.attn.masked_bias')]
        sd_keys_hf = [k for k in sd_keys_hf if not k.endswith('.attn.bias')] # same, just the mask (buffer)
        # Some weights need special handling due to Conv1D vs Linear differences
        transposed = ['attn.c_attn.weight', 'attn.c_proj.weight', 'mlp.c_fc.weight', 'mlp.c_proj.weight']
        
        # Basically the openai checkpoints use a "Conv1D" module, but we only want to use a vanilla
        # Linear. This means that we have to transpose these weight matrices when we import them
...
if ddp:
    ...
    # vanilla, non-DDP run, single GPU for training
    ddp_rank = 0
    ddp_local_rank = 0
    ddp_world_size = 1
    master_process = True
...
parser = argparse.ArgumentParser(description="GPT-2 pretraining with configurable learning rate")
...
B = 64  # micro batch size if A100: 64 1024, if RTX4070 8 256, if B200: 96-128 1024
...
model.to(device)
use_compile = False # torch.compile interfere with HellaSwag eval and Generation. TODO fix
if use_compile:
...
max_lr = args.max_lr  # Use command line argument
min_lr = max_lr * 0.1
warmup_steps = 715
max_steps = 19073  # 1 epoch, for our data is 10B tokens and and batch size is nearly 0.5M tokens
def get_lr(it):
...
log_file = os.path.join(log_dir, f"log_{max_lr}.txt")
    ...
    if step % 250 == 0 or last_step:
        ...
        if master_process:
            ...
            if step > 0 and (step % 5000 == 0 or last_step):
                ...
                checkpoint_path = os.path.join(log_dir, f"model_{max_lr}_{step:05d}.pt")
                checkpoint = {
